...
require_once($CFG->dirroot . '/mod/openmeetings/backup/moodle2/backup_openmeetings_stepslib.php'); // Because it exists (must)

/**
 * openmeetings backup task that provides all the settings and steps to perform one
...
class backup_openmeetings_activity_task extends backup_activity_task {
    ...
    static public function encode_content_links($content) {
        ...
        $base = preg_quote($CFG->wwwroot.'/mod/openmeetings','#');

        //Access a list of all links in a course
        $pattern = '#('.$base.'/index\.php\?id=)([0-9]+)#';
        $replacement = '$@OPENMEETINGSINDEX*$2@$';
        $content = preg_replace($pattern, $replacement, $content);

        //Access the link supplying a course module id
        $pattern = '#('.$base.'/view\.php\?id=)([0-9]+)#';
        $replacement = '$@OPENMEETINGSVIEWBYID*$2@$';
        $content = preg_replace($pattern, $replacement, $content);
